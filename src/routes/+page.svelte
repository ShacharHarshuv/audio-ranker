<script lang="ts">
	...
	const shuffledItems = $derived.by(() =>
		[...data.audioItems]
...
>
	<header class="space-y-2">
		...
		<p class="text-sm text-zinc-600">
			Pick the better clip each round. Ratings update with TrueSkill after every
			choice.
		</p>
	</header>

	{#if data.audioItems.length < 2}
		<section
			class="rounded-xl border border-zinc-200 bg-white p-4 text-sm text-zinc-700 shadow-sm"
		>
			Add at least two audio files to <code>static/audio</code> to start ranking.
		</section>
	{:else}
